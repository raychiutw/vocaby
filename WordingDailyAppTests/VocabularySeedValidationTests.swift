...
final class VocabularySeedValidationTests: XCTestCase {
    func testBundledSeedHasTenItemsPerLevelAndPassesValidation() throws {
        let items = try SeedLoader().loadBundledSeed()

        XCTAssertEqual(items.count, 30)
        XCTAssertEqual(items.filter { $0.level == .basic }.count, 10)
        XCTAssertEqual(items.filter { $0.level == .intermediate }.count, 10)
        XCTAssertEqual(items.filter { $0.level == .advanced }.count, 10)
