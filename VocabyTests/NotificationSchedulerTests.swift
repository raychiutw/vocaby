...
final class NotificationSchedulerTests: XCTestCase {
    func testDailyReminderIncludesTodayDeepLink() {
        ...
        XCTAssertEqual(NotificationScheduler.deepLinkUserInfoKey, "vocaby.internal-url")
        XCTAssertEqual(
    ...
    func testDailyReminderUsesFixedVocabyIdentifierAndRepeatingTime() throws {
        let request = NotificationScheduler().dailyReminderRequest(
            hour: 8,
            minute: 30,
    ...
    func testCancellationOnlyTargetsVocabyReminderIdentifiers() {
        let scheduler = NotificationScheduler()

        XCTAssertEqual(
            NotificationScheduler.reminderRequestIdentifiers,
            ["vocaby.daily-reminder"]
        )
        XCTAssertEqual(
