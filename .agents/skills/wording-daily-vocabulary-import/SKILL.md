...
description: Use when importing or re-importing an external Wording Daily vocabulary source, including Wiktextract, CMUdict, OEWN, CSV, TSV, XLSX, ZIP, TAR, TEI, or JSON; or when producing and auditing rich review JSONL before promoting the bundled bank. 適用於新增詞庫、重跑來源檔、補強多讀音多詞義、翻譯例句題目與審核上架；不適用於 App runtime 下載、登入同步、一般試算表整理或略過權利審核。
version: 2026.7.11
...
- Confirm `WordingDailyApp.xcodeproj/project.pbxproj` contains no `Content/Sources`, `Content/Reviews`, or `VocabularyProvenance` path.
</workflow>
