...
struct SeedLoader {
    ...
    static let sampleItems: [VocabularySeedItem] = [
        ...
        VocabularySeedItem(
            ...
            pronunciationText: "give me a hand",
            quiz: VocabularyQuiz(
                prompt: [
...
enum SeedValidator {
    ...
    private static func validateRequiredFields(_ item: VocabularySeedItem) throws {
        ...
        for languageCode in item.supportLanguageCodes {
            guard item.meaning[languageCode]?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false,
                  item.example.translation[languageCode]?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false,
                  item.quiz.prompt[languageCode]?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false else {
                throw SeedValidationError.missingRequiredField(item.id)
            }
        }
    }
